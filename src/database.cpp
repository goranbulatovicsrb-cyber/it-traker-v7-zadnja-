...
double Database::weeklyEarnings(QDate weekStart)
{
    QString from = weekStart.toString("yyyy-MM-dd");
    QString to   = weekStart.addDays(6).toString("yyyy-MM-dd");
    QSqlQuery q(m_db);
    q.prepare("SELECT COALESCE(SUM(total_price),0) FROM work_entries WHERE user_id=? AND date>=? AND date<=?");
    q.addBindValue(currentUserId()); q.addBindValue(from); q.addBindValue(to);
    if (q.exec() && q.next()) return q.value(0).toDouble();
    return 0.0;
}

int Database::weeklyJobCount(QDate weekStart)
{
    QString from = weekStart.toString("yyyy-MM-dd");
    QString to   = weekStart.addDays(6).toString("yyyy-MM-dd");
    QSqlQuery q(m_db);
    q.prepare("SELECT COUNT(*) FROM work_entries WHERE user_id=? AND date>=? AND date<=?");
    q.addBindValue(currentUserId()); q.addBindValue(from); q.addBindValue(to);
    if (q.exec() && q.next()) return q.value(0).toInt();
    return 0;
}
...
int Database::addTodo(TodoItem& t)
{
    QSqlQuery q(m_db);
    q.prepare("INSERT INTO todos (user_id,text,done,priority,created) VALUES(?,?,?,?,?)");
    q.addBindValue(t.userId); q.addBindValue(t.text);
    q.addBindValue(0); q.addBindValue(t.priority); q.addBindValue(t.created);
    if (!q.exec()) return -1;
    t.id = q.lastInsertId().toInt();
    return t.id;
}

bool Database::setTodoDone(int id, bool done)
{
    QSqlQuery q(m_db);
    q.prepare("UPDATE todos SET done=? WHERE id=? AND user_id=?");
    q.addBindValue(done?1:0); q.addBindValue(id); q.addBindValue(currentUserId());
    return q.exec();
}
...
{
    QSqlQuery q(m_db);
    q.prepare("DELETE FROM todos WHERE id=? AND user_id=?");
    q.addBindValue(id); q.addBindValue(currentUserId());
    return q.exec();
}

bool Database::clearDoneTodos()
{
    QSqlQuery q(m_db);
    q.prepare("DELETE FROM todos WHERE done=1 AND user_id=?");
    q.addBindValue(currentUserId());
    return q.exec();
}

QList<TodoItem> Database::getAllTodos()
{
    QList<TodoItem> list;
    QSqlQuery q(m_db);
    q.prepare("SELECT id,user_id,text,done,priority,created FROM todos WHERE user_id=? ORDER BY done ASC, CASE priority WHEN 'Visok' THEN 0 WHEN 'Srednji' THEN 1 ELSE 2 END, id DESC");
    q.addBindValue(currentUserId());
    q.exec();
    while (q.next()) {
        TodoItem t;
        t.id=q.value(0).toInt(); t.userId=q.value(1).toInt();
        t.text=q.value(2).toString(); t.done=q.value(3).toInt()==1;
        t.priority=q.value(4).toString(); t.created=q.value(5).toString();
        list.append(t);
    }
    return list;
}
