...
void Dashboard::buildUi()
{
    ...
    auto* weekCard = new QFrame;
    weekCard->setObjectName("card");
    auto* weekLay = new QHBoxLayout(weekCard);
    weekLay->setContentsMargins(20,12,20,12);
    weekLay->setSpacing(0);

    auto* weekTitle = new QLabel("📅  Sedmična zarada");
    weekTitle->setStyleSheet("font-weight:700; font-size:13px; color:#f0f6fc;");
    weekLay->addWidget(weekTitle);
    weekLay->addSpacing(24);

    auto mkWeekItem = [&](const QString& label, QLabel*& val) {
        ...
        val = new QLabel("—");
        val->setStyleSheet("font-size:16px; font-weight:700; color:#f0f6fc;");
        l->addWidget(lbl); l->addWidget(val);
        weekLay->addWidget(w);
        weekLay->addSpacing(32);
    };

    mkWeekItem("Ova sedmica",  m_lblThisWeek);
    mkWeekItem("Prošla sedmica", m_lblLastWeek);

    m_lblWeekDiff = new QLabel;
    m_lblWeekDiff->setStyleSheet("font-size:14px; font-weight:700;");
    weekLay->addWidget(m_lblWeekDiff);
    weekLay->addStretch();
    lay->addWidget(weekCard);
    ...
    auto* compareCard = new QFrame;
    compareCard->setObjectName("card");
    auto* compareLay = new QVBoxLayout(compareCard);
    compareLay->setContentsMargins(16,16,16,16);
    auto* compareTitle = new QLabel("📊  Usporedba godina — " +
        QString::number(QDate::currentDate().year()-1) + " vs " +
        QString::number(QDate::currentDate().year()));
    compareTitle->setStyleSheet("font-weight:600; font-size:14px; color:#f0f6fc;");
    compareLay->addWidget(compareTitle);
    m_compareView = new QChartView;
    m_compareView->setRenderHint(QPainter::Antialiasing);
    m_compareView->setMinimumHeight(200);
    compareLay->addWidget(m_compareView);
    lay->addWidget(compareCard);
...
void Dashboard::updateWeeklyStats()
{
    QDate today     = QDate::currentDate();
    QDate thisWeekStart = today.addDays(-(today.dayOfWeek()-1));
    QDate lastWeekStart = thisWeekStart.addDays(-7);

    double thisWeek = Database::instance().weeklyEarnings(thisWeekStart);
    double lastWeek = Database::instance().weeklyEarnings(lastWeekStart);

    m_lblThisWeek->setText(QString("%1 €").arg(thisWeek, 0,'f',2));
    m_lblLastWeek->setText(QString("%1 €").arg(lastWeek, 0,'f',2));

    if (lastWeek > 0) {
        double diff = ((thisWeek - lastWeek) / lastWeek) * 100.0;
        QString arrow = diff >= 0 ? "▲" : "▼";
        QString color = diff >= 0 ? Style::GREEN : Style::RED;
        m_lblWeekDiff->setText(QString("<span style='color:%1'>%2 %3%</span>")
                                   .arg(color).arg(arrow).arg(qAbs(diff),0,'f',1));
    } else if (thisWeek > 0) {
        m_lblWeekDiff->setText(QString("<span style='color:%1'>▲ Nova zarada!</span>").arg(Style::GREEN));
    } else {
        m_lblWeekDiff->setText("<span style='color:#484f58'>Nema podataka</span>");
    }
}

void Dashboard::updateYearCompare()
{
    int currentYear = QDate::currentDate().year();
    int prevYear    = currentYear - 1;

    auto thisYearData = Database::instance().earningsPerMonth(currentYear);
    auto prevYearData = Database::instance().earningsPerMonth(prevYear);

    auto* seriesPrev = new QBarSet(QString::number(prevYear));
    seriesPrev->setColor(QColor("#30363d"));
    seriesPrev->setBorderColor(QColor("#484f58"));

    auto* seriesThis = new QBarSet(QString::number(currentYear));
    seriesThis->setColor(QColor("#1f6feb"));
    seriesThis->setBorderColor(QColor("#58a6ff"));

    for (int m = 1; m <= 12; m++) {
        *seriesPrev << prevYearData.value(m, 0.0);
        *seriesThis << thisYearData.value(m, 0.0);
    }

    auto* series = new QBarSeries;
    series->append(seriesPrev);
    series->append(seriesThis);
    ...
    m_compareView->setChart(chart);
    m_compareView->setBackgroundBrush(QColor("#161b22"));
    m_compareView->setFrameShape(QFrame::NoFrame);
}
