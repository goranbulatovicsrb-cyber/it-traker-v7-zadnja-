...
public:
    ...
    double               unpaidTotal();
    QList<WorkEntry>     overdueEntries(int days = 30);
    int                  workCountThisMonth();
    int                  totalClients();
    QMap<int,double>     earningsPerMonth(int year);
    QMap<QString,double> earningsByType(int year);
    double               weeklyEarnings(QDate weekStart);
    int                  weeklyJobCount(QDate weekStart);

    // ── Todos ─────────────────────────────────────────────────────────────
    int             addTodo(TodoItem& t);
    bool            setTodoDone(int id, bool done);
