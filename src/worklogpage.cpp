...
void WorkLogPage::buildUi()
{
    ...
    auto* btnEmail     = new QPushButton("📧  Email");
    btnEmail->setEnabled(false);
    m_btnEmail = btnEmail;
    auto* btnWhatsApp  = new QPushButton("💬  WhatsApp");
    btnWhatsApp->setEnabled(false);
    m_btnWhatsApp = btnWhatsApp;

    toolRow->addWidget(btnExportCsv);
    toolRow->addWidget(btnInvoice);
    toolRow->addWidget(btnEmail);
    toolRow->addWidget(btnWhatsApp);
    ...
    connect(btnEmail,     &QPushButton::clicked, this, &WorkLogPage::onSendEmail);
    connect(btnWhatsApp,  &QPushButton::clicked, this, &WorkLogPage::onSendWhatsApp);
...
void WorkLogPage::onSelectionChanged()
{
    ...
    m_btnEmail->setEnabled(sel);
    m_btnWhatsApp->setEnabled(sel);
}
...
void WorkLogPage::onSendEmail()
{
    WorkEntry e = selectedEntry();
    if (e.id < 0) return;

    QString subject = QString("Faktura za uslugu — %1").arg(e.workType);
    QString body    = QString(
        "Poštovani %1,\n\n"
        "U prilogu se nalazi faktura za obavljenu uslugu.\n\n"
        "Detalji usluge:\n"
        "  Vrsta: %2\n"
        "  Datum: %3\n"
        "  Opis: %4\n"
        "  Ukupno: %5 €\n\n"
        "Molim vas da platite u roku od 15 dana.\n\n"
        "Hvala na povjerenju!\n"
        "S poštovanjem"
    ).arg(e.clientName)
     .arg(e.workType)
     .arg(e.date.toString("dd.MM.yyyy"))
     .arg(e.description.isEmpty() ? e.workType : e.description)
     .arg(e.totalPrice, 0, 'f', 2);

    // Open default email client
    QString mailtoUrl = QString("mailto:?subject=%1&body=%2")
                            .arg(QString(QUrl::toPercentEncoding(subject)))
                            .arg(QString(QUrl::toPercentEncoding(body)));
    QDesktopServices::openUrl(QUrl(mailtoUrl));
}

void WorkLogPage::onSendWhatsApp()
{
    WorkEntry e = selectedEntry();
    if (e.id < 0) return;
    ...
    QString msg = QString(
        "Pozdrav %1! 👋\n\n"
        "Evo detalja za obavljenu uslugu:\n\n"
        "🔧 *%2*\n"
        "📅 Datum: %3\n"
        "%4"
        "💰 Ukupno: *%5 €*\n\n"
     ...
     .arg(e.workType)
     .arg(e.date.toString("dd.MM.yyyy"))
     .arg(e.description.isEmpty() ? "" : QString("📝 %1\n").arg(e.description))
     .arg(e.totalPrice, 0, 'f', 2);

    // Copy to clipboard + open WhatsApp Web
    QApplication::clipboard()->setText(msg);

    QMessageBox msgBox(this);
    msgBox.setWindowTitle("💬 WhatsApp poruka");
    msgBox.setText("Poruka je kopirana u clipboard!");
    msgBox.setInformativeText("Otvori WhatsApp Web i zalijepi (Ctrl+V) ili otvori WhatsApp Desktop?");
    msgBox.addButton("🌐 WhatsApp Web", QMessageBox::AcceptRole);
    msgBox.addButton("Samo kopiraj", QMessageBox::RejectRole);
